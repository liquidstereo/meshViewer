...
from process.mode.labels import (
    ...
    LBL_PT_CLOUD_RGB, LBL_PT_CLOUD_WHITE,
)
...
def register(p, trigger, set_mode):
    ...
    def _toggle_vtx():
        ...
        if getattr(p, '_n_faces', 1) == 0:
            p._pt_cloud_use_rgb = not getattr(p, '_pt_cloud_use_rgb', True)
            label = (
                LBL_PT_CLOUD_RGB if p._pt_cloud_use_rgb
                else LBL_PT_CLOUD_WHITE
            )
            set_mode(label)
