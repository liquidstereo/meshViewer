...
from configs.defaults import (
    ...
    COLOR_MESH_NO_TEX, ERROR_MSG_DURATION,
)
...
_HELP_TEXT = (
    ...
    'Tab        Actor Visibility Cycle\n'
    'F1-F4      Front/Back/Top/Side View\n'
    'KP+ / -    Inc. / Dec. (Pt. Size)\n'
...
def update_status_text(plotter, idx: int, total: int, fps: float) -> None:
    ...
    bf_str = 'ON' if is_backface else 'OFF'
    ...
    if audio_fps is not None and audio_fps > 0:
        ...
        time_part = ''
    status = (
        f'FRAME. {idx + 1:04d}/{total:04d} | FPS: {fps:.3f}\n'
...
def update_log_overlay(plotter) -> None:
    ...
    if error_msg and (now - error_time) < ERROR_MSG_DURATION:
        ...
        prop.SetColor(*_hex_to_rgb(COLOR_MESH_NO_TEX))
    else:
