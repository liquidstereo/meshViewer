...
def _init_mesh_iso_wire_actors(plotter) -> None:
    ...
    plotter._wire_lut = (
        None if COLOR_WIREFRAME.startswith('#')
        else _make_vtk_lut(COLOR_WIREFRAME)
    )
...
def _init_edge_actor(plotter) -> None:
    plotter._edge_lut = (
        None if COLOR_EDGE.startswith('#')
        else _make_vtk_lut(COLOR_EDGE)
    )
    ...
    edge_actor.GetProperty().SetLineWidth(WIDTH_EDGE)
...
def _init_vtx_actors(plotter) -> None:
    ...
    vtx_pt_mapper.SetRelativeCoincidentTopologyPointOffsetParameter(-1.0)
...
def _init_fnormal_actor(plotter) -> None:
    ...
    fnormal_mapper.SetLookupTable(_make_vtk_lut(FNORMAL_CMAP))
...
def _init_pick_actor(plotter) -> None:
    ...
    pick_mapper.SetRelativeCoincidentTopologyPointOffsetParameter(-1.0)
