...
from configs.keybinding import TURNTABLE_STEP
import traceback
...
def render_loop(plotter, buffer) -> None:
    ...
    ui_time = last_anim_time = last_update_time = 0.0
    total = buffer.total
    ...
    while plotter.render_window is not None:
        ...
        if not style_needed and _is_cam_dependent_mode(plotter):
        ...
        if style_needed or anim_fired:
            ...
            _update_seq(plotter, plotter._idx)
        ...
        if curr - last_update_time >= ui_interval:
            t0 = time.perf_counter()
            plotter.update(force_redraw=False)
        ...
        if is_playing != _prev_playing:
            ...
            if is_playing:
                ...
                plotter.renderer.SetUseFXAA(RENDER_FXAA)
